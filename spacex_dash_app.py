...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        data = df.groupby( 'Launch Site' )[ 'class' ].value_counts( ).reset_index( name = 'class_count' )
        fig = px.pie( data, values='class_count',
        names="Launch Site",
        ...
        fig = px.pie( data, values = 'class_count',
        names = [ '1', '0' ],
        title = 'Success vs Failure for Site {}'.format( entered_site ) )
        return fig
...
def get_scat_plot( entered_site, payload_range ):
    ...
    if( entered_site == 'ALL' ):
        ...
        xaxis_title = 'Payload Mass [kg]', yaxis_title = 'Success Class' )
        return fig
    else:
        data = df[ df[ 'Launch Site' ] == entered_site ]
        fig = px.scatter( data, x = 'Payload Mass (kg)', y = 'class' ,
        color = 'Booster Version Category' )
        fig.update_layout( title = 'Correlation Between Payload and Success for Site {}'.format( entered_site ),
        xaxis_title = 'Payload Mass [kg]', yaxis_title = 'Success Class' )
        return fig
